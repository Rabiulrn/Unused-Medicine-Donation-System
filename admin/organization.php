...
<head>
    ...
    <title>Organization Donation List</title>
...
<body>
    ...
    <h3 class="text-center text-secondary mt-5">Organization Donoation Proposals</h3>
    <div class="text-center"><a href="#scroll_down" class="btn btn-warning scroll_down"><i class="fas fa-sort-down"></i>Active Scroll Down</a></div>
        ...
        <tbody class="text-center">
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    echo '<input type="hidden" name="orgID" value=' . $row["orgID"] . '>';
    ...
    {
        // code...
        $sql = "SELECT orgID,name,email,location,upzilla,district,medicine,quantity,phone,daate FROM organizationdonation WHERE orgID={$_REQUEST['orgID']}";
        $res = $conn->query($sql);
        $row = $res->fetch_assoc();
    }
    ...
    if (isset($_REQUEST['close'])) {
        $sql = "DELETE FROM organizationdonation WHERE orgID = {$_REQUEST['orgID']}";
    ...
    if (isset($_REQUEST['assign'])) {
        $sql = "SELECT orgID,name,email,location,upzilla,district,medicine,quantity,phone,daate FROM organizationdonation where orgID={$_REQUEST['id']}";
        $res = $conn->query($sql);
        $row = $res->fetch_assoc();
        ...
        //insertion....
        $sql = "INSERT INTO assign_org(donorID,donor,medicine,quantity,`address`,upzilla,district,ngo,phone,date) VALUES('$id','$donor','$medicine','$quantity','$address','$upzilla','$district','$ngo','$phone','$date')";
        $result = $conn->query($sql);
        if ($result == true) {
    ...
    ?>
        ...
        ?>
        <?php
        $sql = "UPDATE organizationdonation SET status = 'Assigned To NGO' WHERE orgID={$_REQUEST['id']}";
        $res = $conn->query($sql);
        ?>
                    ...
                    <div class="form-group p-5">
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-2">
                                ...
                                <input type="text" name="id" class="form-control mt-1 mb-2 text-center text-danger" value="<?php if (isset($_REQUEST['orgID'])) echo $_REQUEST['orgID']; ?>" readonly>
                            </div>

                            <div class="col-sm-10">
                                <label for="name">Organization Donor</label><br>
                        ...
                        <div class="row">
                            <div class="col-sm-6">
                                ...
                                <input type="text" name="address" class="form-control mt-1 mb-2" value="<?php if (isset($_REQUEST['address'])) echo $row['location']; ?>">
                            </div>
                            <div class="col-sm-6">
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-6">
                                <a href="organization.php" class="btn btn-warning "><i class="fas fa-sync pr-2"></i>Refresh</a>
                            </div>
                            <?php
                            if (isset($msg)) {
                                // code...
