...
<body>
    ...
    <h3 class="text-center text-secondary mt-5">Pharmaceutical Users List</h3>
    ...
    <table class="table table-bordered table-striped mt-5">
        ...
        <tbody class="text-center">
            ...
            <?php
            $sql = "SELECT * FROM pharmalogin_tb";
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    echo '<tr><td>' . $row["pha_id"] . '</td><td>';
                    echo $row["pha_name"] . '</td><td>';
                    echo $row["pha_email"] . '</td><td>';

                    echo '<form method="POST" >';
                    echo '<input type="hidden" name="id" value=' . $row["pha_id"] . '>';
    ...
    if (isset($_REQUEST['close'])) {
        $sql = "DELETE FROM pharmalogin_tb WHERE pha_id = {$_REQUEST['id']}";
