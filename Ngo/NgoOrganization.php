...
<body>
    ...
    <h3 class="text-center text-secondary mt-5">Organization Donoation Proposals:<b> <?php echo $n ?></b></h3>
    ...
    <table class="table table-bordered table-striped mt-5">
        ...
        <tbody class="text-center">
            ...
            $sql = "SELECT * FROM assign_org WHERE ngo='$ngoname' ";
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    // code...
    ...
    {
        // code...
        $sql = "SELECT donorID,donor,medicine,quantity,address,upzilla,district,ngo,phone,date FROM assign_org WHERE donorID={$_REQUEST['donorID']}";
        $res = $conn->query($sql);
        $row = $res->fetch_assoc();
    }
    ...
    if (isset($_REQUEST['close'])) {
        $sql = "DELETE FROM assign_org WHERE donorID = {$_REQUEST['donorID']}";
    ...
    if (isset($_REQUEST['assign'])) {
        ...
        //insertion....
        $sql = "INSERT INTO executive_assign_org(donorID,donor,medicine,quantity,address,upzilla,district,executive,phone,daate) VALUES('$id','$donor','$medicine','$quantity','$address','$upzilla','$district','$executive','$phone','$date')";
        $result = $conn->query($sql);
        if ($result == true) {
    ...
    ?>
        ...
        ?>
        <?php
        $sql = "UPDATE assign_org SET status = 'Assigned To Executive' WHERE donorID={$_REQUEST['donorID']}";
        $res = $conn->query($sql);
        $sql = "UPDATE organizationdonation SET status = 'Assigned To Executive' WHERE orgID={$_REQUEST['donorID']}";
        $res = $conn->query($sql);
        ?>

        <script>
            window.location.href = 'NgoOrganization.php';
        </script>
                    ...
                    <div class="form-group p-5">
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-2">
                                ...
                                <input type="text" name="donorID" class="form-control mt-1 mb-2 text-center text-danger" value="<?php if (isset($_REQUEST['donorID'])) echo $_REQUEST['donorID']; ?>" readonly>
                            </div>

                            <div class="col-sm-10">
                                <label for="name"> Donor</label><br>
                                <input type="text" name="donor" class="form-control mt-1 mb-2" value="<?php if (isset($_REQUEST['donor'])) echo $row['donor']; ?>">
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-6">
                                <a href="NgoOrganization.php" class="btn btn-warning "><i class="fas fa-sync pr-2"></i>Refresh</a>
                            </div>
                            <?php
                            if (isset($msg)) {
