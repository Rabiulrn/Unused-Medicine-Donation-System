...
<head>
    ...
    <script>
        function print_donation(X) {
            window.location.href = 'PrintIndi.php?id=' + X;
...
<body>
    ...
    <table id="donation" class="table table-bordered table-striped mt-5">
        ...
        <tbody class="text-center">
            ...
            $sql = "SELECT * FROM executive_assign_indi WHERE executive='$Exemail'";
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    // code...
    ...
    if (isset($_REQUEST['close'])) {
        $sql = "DELETE FROM executive_assign_indi WHERE donorID = {$_REQUEST['donorID']}";
    ...
    if (isset($_REQUEST['assign'])) {
        ...
        //update
        $sql = "UPDATE executive_assign_indi SET status = 'Donation Accepted' WHERE donorID={$_REQUEST['donorID']}";
        $res = $conn->query($sql);
        if ($result == true) {
    ?>
        ...
        ?>
        <?php
        $sql = "UPDATE assign_indi SET status = 'Donation Accepted' WHERE donorID={$_REQUEST['donorID']}";
        $res = $conn->query($sql);
        $sql = "UPDATE individualdonation SET status = 'Donation Accepted' WHERE donorID={$_REQUEST['donorID']}";
        $res = $conn->query($sql);


        ?>
        <script>
            window.location.href = 'ExecutiveIndividual.php';
        </script>
                ...
                <form method="POST">
                    <div class="form-group p-5">
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-6">
                                <a href="ExecutiveIndividual.php" class="btn btn-warning "><i class="fas fa-sync pr-2"></i>Refresh</a>
                            </div>
                            <?php
                            if (isset($msg)) {
