...
<head>
    ...
    <script>
        function print_donation(X) {
            window.location.href = 'PrintOrg.php?id=' + X;
...
<body>
    ...
    <h3 class="text-center text-black mt-5">Organization Donoation Proposals: <b> <?php echo $n ?></b></h3>
    ...
    <table class="table table-bordered table-striped mt-5">
        ...
        <tbody class="text-center">
            ...
            $sql = "SELECT * FROM executive_assign_org WHERE executive='$Exemail'";
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    // code...
    ...
    {
        // code...
        $sql = "SELECT donorID,donor,medicine,quantity,`address`,upzilla,district,executive,phone,daate,`status` FROM executive_assign_org WHERE donorID={$_REQUEST['donorID']}";
        $res = $conn->query($sql);
        $row = $res->fetch_assoc();
    }
    ...
    if (isset($_REQUEST['close'])) {
        $sql = "DELETE FROM executive_assign_org WHERE donorID = {$_REQUEST['donorID']}";
    ...
    if (isset($_REQUEST['assign'])) {
        ...
        //update
        $sql = "UPDATE executive_assign_org SET status = 'Donation Accepted' WHERE donorID={$_REQUEST['donorID']}";
        $res = $conn->query($sql);
        if ($result == true) {
    ?>
        ...
        ?>
            ...
            <script>
                alert('Failed to Collect Donation');
            </script>
        <?php
        }
        ?>
        <?php
        $sql = "UPDATE assign_org SET status = 'Donation Accepted' WHERE donorID={$_REQUEST['donorID']}";
        $res = $conn->query($sql);


        $sql = "UPDATE organizationdonation SET status = 'Donation Accepted' WHERE orgID={$_REQUEST['donorID']}";
        $res = $conn->query($sql);


        ?>
        <script>
            window.location.href = 'ExecutiveOrg.php';
        </script>
                ...
                <form method="POST">
                    <div class="form-group p-5">
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-6">
                                <a href="ExecutiveOrg.php" class="btn btn-warning "><i class="fas fa-sync pr-2"></i>Refresh</a>
                            </div>
                            <?php
                            if (isset($msg)) {
