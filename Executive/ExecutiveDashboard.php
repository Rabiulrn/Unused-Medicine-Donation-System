...
<body>
    ...
    <div class="part1 container mt-5 mb-5">
        <?php
        $sql = "SELECT e_email,e_name,ngo_name FROM executivelogin_tb WHERE e_email='" . $ExecutiveEmail . "' limit 1";

        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {

                echo '<h4 style="color: #007CC7"> Executive: <b> ' . $row['e_name'] . '<span style="color:green"> - ' . $row['ngo_name'] . '</span></b></h4>';
            }
        }
        ?>
            ...
            <div class="col-lg-12">
                <div class="card text-center text-light bg-info p-2">
                    ...
                    <div class="card-body justify-center">
                        ...
                        <?php
                        $sql = "SELECT count(donorID) AS total FROM executive_assign_request WHERE executive ='$Exe_email'";
                        $res = $conn->query($sql);
                        $value = $res->fetch_assoc();
                        $total_count = $value['total'];
                        ...
                        ?>
                        <a class="btn btn-outline-dark text-light" style="width:100px" href="ExecutiveRequest.php">View</a>
                    </div>
                </div>
            </div>
        </div>
