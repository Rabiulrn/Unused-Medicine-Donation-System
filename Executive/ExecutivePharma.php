...
<head>
    ...
    .scroll_down {
      ...
      function print_donation(X){ 
        window.location.href = 'PrintPharma.php?id='+ X;
...
{
    // code...
    $sql="SELECT donorID,donor,medicine,quantity,`address`,upzilla,district,executive,phone,daate,`status` FROM executive_assign_pharma WHERE donorID={$_REQUEST['donorID']}";
    $res=$conn->query($sql);
    $row=$res->fetch_assoc();
}
        ...
        if(isset($_REQUEST['close'])){
            $sql = "DELETE FROM executive_assign_pharma WHERE donorID = {$_REQUEST['donorID']}";
          ...
          echo "Unable to Delete Data";
			...
			if (isset($_REQUEST['assign'])) {
				...
				//update
                $sql="UPDATE executive_assign_pharma SET status = 'Donation Accepted' WHERE donorID={$_REQUEST['donorID']}";
                $res=$conn->query($sql);
                if($result==true)
				{
                    ?>
                ...
                ?>
               <?php
                $sql="UPDATE assign_pharma SET status = 'Donation Accepted' WHERE donorID={$_REQUEST['donorID']}";
                $res=$conn->query($sql);


                $sql="UPDATE pharmadonation SET status = 'Donation Accepted' WHERE pharmaID={$_REQUEST['donorID']}";
                $res=$conn->query($sql);

                
                ?>      
               <script>
                window.location.href = 'ExecutivePharma.php';
